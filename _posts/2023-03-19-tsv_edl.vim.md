...
2019年我剪了十几期podcast。一开始很兴奋，后来很疲惫，最后几期是由我室友(WiFe)帮我剪的。
...
他们用eletron实现了一套很fancy的东西，九个人的团队做的。（我2023看再去看页面已经离职得只剩一位了）

后来descript、adobe podcast都出了类似的方法。但都是租用式服务，或者不支持中文，用着不开心。

所以我自己用vim实现了一个。

# 摘要

- 定义了一种tsv格式（由srt一比一转过来）tsv实际上就是tab分隔的spreadsheet文件，可以直接用excel打开。
- vim可以方便地编辑这个tsv，并唤起mpv来预览。
- 选中行按空格，调用tsv2roughcut（python脚本，调ffmpeg）导出mp4；
- 选中行按x，导出成fcpxml文件，送进fcpx或达芬奇再微调
- vim里按两下`\\`，会后台调起mpv播放器，通过unix socket通信，vim上下动光标，自动socat给mpv去seek。
...
![](https://github.com/scateu/tsv_edl.vim/blob/main/tsv_edl_flow.png)
...
```
 ...
 - 所有不带EDL行都可作为注释。Markdown/org mode式折叠，按Tab键即可。
...
## 4. Cherrypick / Pancake Editing
 - 按`\P`或`\p`，出一个新的vim tab，上下按光标（也可以先打开mpv预览），选到如意的，按回车，当前行即被粘到另一个tab里。本行后来也会变成`---`以便你知道这行已经被用了。

## 5. B roll支持
 - 可以给podcast配图
 - 可以给footage加B roll（避免全是talking heads）

## 6. HTTP素材导入
 - 如果被竖线包围的clipname以http开头，tsv2roughcut（绑到了V键选后按空格）会自动用youtube-dl等工具帮你下回来并合成。而且只下其中要用的那段，而不是整个下回来。
 - 如果觉得下载动作太宝贵，tsv2roughcut会问你要不要删掉tmp文件夹，此时可以抢救
...
## 9. 小工具
 ...
 - macOS的shortcuts: 在README.shortcut.md里。可以用。凑合用。图形界面了算是。
 - 其它工具都在utils目录了。
...
| mm    | 行mark文字切断点，再左右找时间，按竖线切。特别是在时间推断不准的时候用，很有效。 |
